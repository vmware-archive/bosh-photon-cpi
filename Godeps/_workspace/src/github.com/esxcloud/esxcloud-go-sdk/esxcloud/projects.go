...
func (api *ProjectsAPI) Delete(projectID string) (task *Task, err error) {
	res, err := rest.Delete(api.client.httpClient, api.client.Endpoint+"/v1/projects/"+projectID)
	if err != nil {
		return
	}
...
func (api *ProjectsAPI) CreateDisk(projectID string, spec *DiskCreateSpec) (task *Task, err error) {
	...
	res, err := rest.Post(api.client.httpClient, api.client.Endpoint+"/v1/projects/"+projectID+"/disks", bytes.NewReader(body))
	if err != nil {
		return
	}
	defer res.Body.Close()
...
func (api *ProjectsAPI) CreateVM(projectID string, spec *VmCreateSpec) (task *Task, err error) {
	...
	res, err := rest.Post(api.client.httpClient, api.client.Endpoint+"/v1/projects/"+projectID+"/vms", bytes.NewReader(body))
	if err != nil {
		return
	}
	defer res.Body.Close()
