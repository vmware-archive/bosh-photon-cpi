...
var _ = Describe("VM", func() {
	...
	Describe("PowerOnOff Vms", func() {
		It("Power on, off succeeds", func() {
			...
			powerOnTask, err := client.VMs.Operation(task.Entity.ID, &VmOperation{Operation: "START_VM"})
			GinkgoT().Log(err)
			Expect(err).Should(BeNil())
			...
			powerOffTask, err := client.VMs.Operation(task.Entity.ID, &VmOperation{Operation: "STOP_VM"})
			GinkgoT().Log(err)
			Expect(err).Should(BeNil())
	...
	Describe("GetNetworks", func() {
		It("GetNetworks returns a completed task", func() {
			...
			server.SetResponseJson(200, mockTask)
			task, err = client.VMs.GetNetworks(task.ID)
			task, err = client.Tasks.Wait(task.ID)
			GinkgoT().Log(err)
			Expect(err).Should(BeNil())
			Expect(task).ShouldNot(BeNil())
